...
export default function Testimonials() {
  ...
  const testimonialGroup = (copy: string, hidden = false) => (
    <div className="testimonialGroup" aria-hidden={hidden || undefined}>
      {items.map((item) => (
        <article className="testimonialCard" key={`${copy}-${item.id}`}>
  ...
  return (
    <section className="testimonials" id="depoimentos">
      <div className="testimonialIntro">
        <span>03 — QUEM TREINA COM A GENTE</span>
        <h2>EXPERIÊNCIAS NO TATAME.</h2>
        {items.length > 0 ? (
          <div className="testimonialCarousel" aria-label="Depoimentos de alunos">
            <div className="testimonialTrack">
              {testimonialGroup("original")}
              {testimonialGroup("copy", true)}
            </div>
          </div>
        ) : (
          <p className="testimonialEmpty">Treina com a gente? Deixe sua experiência conosco e inspire outras pessoas a conhecerem a Escola Eyroff.</p>
        )}
      </div>

      <form className="feedbackForm" onSubmit={send}>
        <h3>DEIXE SUA EXPERIÊNCIA CONOSCO</h3>
        <p>Conte como está sendo sua experiência na Escola Eyroff.</p>
        ...
        <button className="button">ENVIAR DEPOIMENTO →</button>
